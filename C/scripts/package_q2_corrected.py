...
def package(repo: Path, run: Path, encoder: Path, output: Path):
    ...
    for name in ("__init__.py", "run_q2_corrected.py", "infer_q2_corrected.py",
                 "analyze_q2_corrected.py", "package_q2_corrected.py"):
    ...
    report = {
        "data": {"source": "provided CMU-MOSEI aligned_50 only",
                 "train": 3395, "valid": 728, "attachment3": 30,
...
本包使用附件二对齐版训练集3395条学习参数、验证集728条选模型，对附件三30条完成最终推理。文本、音频和视觉输入固定为50个对齐位置；三种模态特征在相同原始位置进行连续区间缺失。
...
完整缺失视图会占用较多临时磁盘空间；基础缓存和实验缓存可在复现后删除，不需放入本包。验证网格完整结果在 `missing_grid.csv` 和 `served_grid.csv`，逐样本验证预测、错误分析和图表也随包提供。模型和运行环境版本见 `q2_corrected.json` 与 `requirements.txt`。
'''
