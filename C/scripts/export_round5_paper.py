"""Export fifth-round Markdown with embedded figures and editable Word equations."""
import sys
import re
...
SOURCE=ROOT/'docs/C/paper/第五轮独立专家与决策融合实验论文草稿.md'

def equation(block):
    if 'L=' in block:
        terms=[text('L = '),summation('s=1','11',[sub('a','s'),text('['),text('−'),fraction([summation('i=1','N',[sub('w','yᵢ'),text(' log '),sub('p','si,yᵢ')])],[summation('i=1','N',[sub('w','yᵢ')])]),text(' + '),fraction([text('0.8')],[text('N')]),summation('i=1','N',[sub('H','1'),text('('),sub('r','si'),text(' − '),sub('s','i'),text(')')]),text(']')])]
    elif 'g_{ie}=' in block:
        terms=[sub('g','ie'),text(' = '),fraction([sub('m','ie'),text(' exp('),sub('z','ie'),text(')')],[summation('k∈{T,AV}','',[sub('m','ik'),text(' exp('),sub('z','ik'),text(')')])])]
    else:
        terms=[sub('p','i'),text(' = '),summation('e∈{T,AV}','',[sub('g','ie'),sub('p','ie')]),text('，  '),sub('r','i'),text(' = '),summation('e∈{T,AV}','',[sub('g','ie'),sub('r','ie')])]
...
def main():
    ...
    for block in SOURCE.read_text(encoding='utf-8').strip().split('\n\n'):
        if block.startswith('# '): doc.add_heading(block[2:],0)
            ...
            widths=[3.3,4.2,3.2,3.2,3.1] if len(doc.tables)==1 else [5.5,3.6,3.6,4.3]
            ...
            p.add_run().add_picture(str(path),width=Cm(17))
        else:
            caption=block.startswith('**图C5-')
            p=doc.add_paragraph(style='Caption' if caption else 'Normal');inline(p,block)
            if caption:p.paragraph_format.keep_together=True
    p=sec.footer.paragraphs[0];p.alignment=1;p.add_run('第五轮实验临时稿 · ')
    f=OxmlElement('w:fldSimple');f.set(qn('w:instr'),'PAGE');p._p.append(f)
    doc.core_properties.title='第五轮独立专家与决策融合实验论文草稿'
    doc.core_properties.subject='独立专家、折外决策融合、准确率收益与中性识别退化'
    ...
    math_blocks=check._element.xpath('.//m:oMathPara')
    assert len({''.join(block.itertext()) for block in math_blocks})==3
    assert not any('$$' in p.text or '![' in p.text for p in check.paragraphs)
    print('Exported fifth-round Word: 5 figures, 2 tables, 3 native equations.')
