"""Export fourth-round Markdown with embedded figures and editable Word equations."""
import sys
import re
...
SOURCE=ROOT/'docs/C/paper/第四轮结构优化实验论文草稿.md'

def equation(block):
    if 'L=' in block:
        terms=[text('L = −'),fraction([summation('i=1','N',[sub('w','yᵢ'),text(' log '),sub('p','i,yᵢ')])],[summation('i=1','N',[sub('w','yᵢ')])]),text(' + '),fraction([text('0.8')],[text('N')]),summation('i=1','N',[sub('H','1'),text('('),sub('ŷ','i'),text(' − '),sub('s','i'),text(')')])]
    elif 'H=' in block:
        terms=[text('H = LN(Q + MHA(Q, K, V)) ⊙ M')]
    else:
        terms=[sub('p','0'),text(' = (1 − σ(q))(1 − σ(r)),  '),sub('p','1'),text(' = σ(q),  '),sub('p','2'),text(' = (1 − σ(q))σ(r)')]
...
def main():
    ...
    for block in SOURCE.read_text(encoding='utf-8').strip().split('\n\n'):
        if block.startswith('# '): doc.add_heading(block[2:],0)
            ...
            widths=[3.3,2.1,4.6,3.5,3.5] if len(doc.tables)==1 else [5.5,3.6,3.6,4.3]
    ...
    print('Exported fourth-round Word: 5 figures, 2 tables, 3 native equations.')
