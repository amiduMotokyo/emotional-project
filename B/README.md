...
修订后的问题二流程发现int8 ONNX编码输出随批次大小变化，因此附件2训练、验证缓存和附件3逐条推理统一采用单样本调用。缺失文本在送入编码器前替换为 `[UNK]`；实现和指标见[问题二输入一致性实验](../docs/C/experiments/q2_input_consistent_experiments.md)。
