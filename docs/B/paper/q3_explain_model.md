...
为检验事后解释之外的可训练解释性方法，冻结已选MiniLM编码器，从上述Fusion checkpoint继续训练门控。以训练样本真值类logit的三模态删除效应的正值归一化分布 `q` 为停止梯度目标，在原分类和强度损失上增加 `λ KL(q||g)`。使用与部署ONNX编码器完全一致的逐条重编码缓存；旧文本缓存不一致，不能用于这一对照。

在 `λ=1`、seed 20260925、第1轮时，验证集ACC仍为464/728；门控最大模态与验证样本预测类删除效应最大模态的吻合率由89.15%提高到90.93%，平均L1差异由0.56179降到0.52759。但Macro-F1由0.60008略降到0.59982，MAE由0.59605升到0.59790，高置信错误由47增到61。附件4的20条极性不变，样本04因音频/视觉作用接近并列而由“主音频”变为“主视觉”。更强的约束会牺牲验证ACC，另外两个随机种子也未稳定超过原版。因此正式全量预测仍以冻结原版为主，解释约束版作为已训练的消融对照，不能据此声称预测性能提高。详细配置、逐轮结果和全量实验版解释见 [解释约束训练实验](../experiments/q3_explanation_constraint_20260925.md)。
